...
const Summary = () => {
  ...
  const specificOutreach = shouldShowSpecificOutreach ? outreachActivities?.find(item => item.campaign_id === campaignId && item.influencer_id === influencerId) : null;

  // Use specific outreach communication logs if viewing specific outreach
  const {
    communicationLogs
  } = useCommunicationLogs(specificOutreach?.outreach_id);
  const {
    deals
  } = useDealsContracts();
  ...
  const filteredOutreach = shouldShowSpecificOutreach && specificOutreach ? [specificOutreach] : outreachActivities?.filter(item => {
  ...
  const activeCampaigns = outreachActivities?.filter(item => ["AI Drafting", "AI Reaching Out", "Waiting for Response", "Response - Positive Interest", "Negotiating"].includes(item.status)) || [];
  const attentionRequired = outreachActivities?.filter(item => ["Issue: AI Needs Human Help"].includes(item.status)) || [];
  const recentSuccesses = outreachActivities?.filter(item => ["Deal Finalized", "Contract Signed"].includes(item.status)) || [];
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      "AI Drafting": {
        color: "bg-blue-100 text-blue-800",
        text: "AI Drafting (GPT-4)"
      },
      "AI Reaching Out": {
        color: "bg-blue-100 text-blue-800",
        text: "AI Reaching Out"
      },
      "Waiting for Response": {
        color: "bg-yellow-100 text-yellow-800",
        text: "Waiting for Response"
      },
      "Response - Positive Interest": {
        color: "bg-green-100 text-green-800",
        text: "Response - Positive Interest"
      },
      "Negotiating": {
        color: "bg-orange-100 text-orange-800",
        text: "Negotiating (AI GPT-4)"
      },
      "Deal Finalized": {
        color: "bg-green-100 text-green-800",
        text: "Deal Finalized"
      ...
      "Issue: AI Needs Human Help": {
        color: "bg-red-100 text-red-800",
        text: "Issue: AI Needs Human Help"
      }
    };
                ...
                <Tabs defaultValue="progress" className="w-full">
                  ...
                  <TabsContent value="communication" className="space-y-6">
                    <div className="space-y-4 max-h-96 overflow-y-auto bg-gray-50 rounded-lg p-6">
                      {communicationLogs && communicationLogs.length > 0 ? communicationLogs.map(log => <div key={log.log_id} className="flex space-x-4 bg-white rounded-lg p-4 shadow-sm">
                            ...
                            <div className="flex-1">
                              ...
                              {log.subject_or_summary && <p className="text-sm font-medium text-gray-800 mb-1">{log.subject_or_summary}</p>}
                              {log.content_transcript && <p className="text-sm text-gray-600">{log.content_transcript}</p>}
                            </div>
                          ...
                          <CardContent>
                            ...
                            <div className="mt-6 pt-6 border-t border-green-200">
                              <div className="flex justify-between items-center">
                                {deal.contract_document_url && <Button variant="outline" className="border-green-300 text-green-700 hover:bg-green-50">
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <Card className="border-blue-200 bg-blue-50">
                ...
                <CardContent>
                  ...
                  <div className="space-y-2">
                    {activeCampaigns.slice(0, 2).map(item => <div key={item.outreach_id} className="text-xs p-2 bg-blue-100 rounded border border-blue-200">
                        <span className="font-medium">{getCampaignName(item.campaign_id)}</span> - {getInfluencerName(item.influencer_id)}
                      </div>)}
                    {activeCampaigns.length > 2 && <p className="text-xs text-blue-600">+{activeCampaigns.length - 2} more</p>}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-orange-200 bg-orange-50">
                ...
                <CardContent>
                  ...
                  <div className="space-y-2">
                    {attentionRequired.slice(0, 2).map(item => <div key={item.outreach_id} className="text-xs p-2 bg-orange-100 rounded border border-orange-200">
                        <span className="font-medium">{item.status}</span> - {getCampaignName(item.campaign_id)}
                      </div>)}
                    {attentionRequired.length === 0 && <p className="text-xs text-orange-600">All systems running smoothly</p>}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-green-200 bg-green-50">
                ...
                <CardContent>
                  ...
                  <div className="space-y-2">
                    {recentSuccesses.slice(0, 2).map(item => <div key={item.outreach_id} className="text-xs p-2 bg-green-100 rounded border border-green-200">
                        <span className="font-medium">{getCampaignName(item.campaign_id)}</span> - {getInfluencerName(item.influencer_id)}
                      </div>)}
                    {recentSuccesses.length === 0 && <p className="text-xs text-green-600">Keep up the great work!</p>}
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Select value={filters.campaign} onValueChange={value => setFilters({
              ...filters,
              campaign: value
            })}>
                <SelectTrigger>
              ...
              <Select value={filters.method} onValueChange={value => setFilters({
              ...filters,
              method: value
            })}>
                <SelectTrigger>
          ...
          <div className="bg-white rounded-lg shadow-sm border">
            ...
            <div className="p-6">
              {filteredOutreach.length > 0 ? <div className="space-y-4">
                  {filteredOutreach.map(outreach => <Card key={outreach.outreach_id} className="hover:shadow-md transition-shadow border border-gray-200">
                              ...
                              <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                                ...
                                <div className="space-y-6">
                                  <div className="p-4 bg-gray-50 rounded-lg">
                                    ...
                                    {outreach.notes_and_alerts && <div className="mt-3">
